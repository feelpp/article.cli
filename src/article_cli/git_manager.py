...
from .zotero import print_success, print_error, print_warning, print_info
...
class GitManager:
    ...
    def setup_hooks(self) -> bool:
        ...
        try:
            ...
            if not git_head_info.exists():
                ...
                subprocess.run(['git', 'add', 'gitHeadLocal.gin'], check=True, cwd=self.repo_root)
                subprocess.run(['git', 'commit', '-m', 'Created gitHeadLocal.gin for initial setup'],
    ...
    def create_release(self, version: str, auto_push: bool = False) -> bool:
        ...
        try:
            # Check if tag exists
            result = subprocess.run(['git', 'rev-parse', version],
            ...
            if git_head_info.exists():
                ...
                subprocess.run(['git', 'add', 'gitHeadLocal.gin'],
                             check=True, cwd=self.repo_root)
                subprocess.run(['git', 'commit', '-m', f'Updated gitHeadLocal.gin for release {version}'],
                             check=True, cwd=self.repo_root)
                subprocess.run(['git', 'tag', '-f', '-a', version, '-m', f'Release {version}'],
                             check=True, cwd=self.repo_root)
            ...
            if auto_push:
                try:
                    subprocess.run(['git', 'push', 'origin', '--follow-tags'],
                                 check=True, cwd=self.repo_root)
        ...
        try:
            ...
            if delete_remote:
                try:
                    ...
                    print_info(f"Delete manually with: git push origin --delete {version}")
            else:
...
# Import Colors class for consistent styling
from .zotero import Colors